...
const PrivacyPolicyPublic = () => {
  ...
  const fetchPolicy = async (policyType) => {
    ...
    setError(null);
    try {
      const res = await getPublicCmsContent(policyType, "privacy_policy");
      setContent(res.data?.content || "<p class='text-gray-500 italic'>No policy content defined yet.</p>");
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-4xl min-h-[400px] flex flex-col">
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center py-20">
            ...
            <p className="text-slate-500 mt-4 font-medium">Fetching latest policy...</p>
          </div>
        ) : error ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <button
              onClick={() => fetchPolicy(activeType)}
