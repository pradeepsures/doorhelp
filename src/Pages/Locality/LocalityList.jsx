...
import { getLocalities, deleteLocality, updateLocality } from "../../Services/localityService";
...
export default function LocalityList() {
  ...
  const fetchLocalities = async (currentPage, searchQuery, currentStatus) => {
    try {
      setLoading(true);
      const res = await getLocalities(currentPage, 10, searchQuery, currentStatus);
      if (res.success) {
        setLocalities(res.data || []);
        setTotalPages(res.pagination?.totalPages || 1);
      } else {
        toast.error(res.message || "Failed to load localities");
      }
    } catch (error) {
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      fetchLocalities(page, search, statusFilter);
    }, 500);
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    ...
    try {
      const res = await updateLocality(id, { status: newStatus });
      if (res.success) {
        toast.success(`Locality status updated to ${newStatus}`);
        fetchLocalities(page, search, statusFilter);
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await deleteLocality(id);
      if (res.success) {
  ...
  const handleExport = () => {
    ...
    const formattedData = localities.map((loc, index) => ({
      "Sr No": index + 1,
      "Locality Name": loc.name || "",
      Status: loc.status === "active" ? "Active" : "Inactive",
      "Created At": loc.createdAt ? formatDate(loc.createdAt) : "",
    }));
    exportToExcel(formattedData, "Localities_List");
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
          ...
          <div className="flex flex-wrap gap-3 w-full sm:w-auto">
            ...
            {/* Create */}
            <button
              onClick={() => navigate("/home/locality/create")}
              ...
              <table className="min-w-full border-collapse">
                <thead className="text-white text-sm uppercase">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left font-medium tracking-wider bg-theme-gradient-horizontal">Locality Name</th>
                              ...
                              <ul className="absolute right-0 mt-2 w-36 bg-white border border-gray-100 rounded-lg shadow-xl text-sm z-50 overflow-hidden text-left">
                                <li>
                                  <button
                                    onClick={() => {
                                      setOpenMenuId(null);
                                      navigate(`/home/locality/edit/${row._id}`);
                                    }}
