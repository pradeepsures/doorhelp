...
// Reusable DetailRow Component matching the user's booking details screenshot layout
    ...
    <div className="w-full min-h-screen bg-[#F8FAFC] p-3 md:p-5 text-slate-700 space-y-4">
      ...
      {loading ? (
        ...
        <motion.div 
          ...
          className="space-y-4"
        >
          {/* CARD 1: GENERAL INFORMATION (Horizontal Stacked Card) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1">
              ...
              <div className="space-y-1">
                <DetailRow 
                  ...
                  value={subcategory.categoryId?.name || "N/A"} 
                />
                <DetailRow 
                  label="Status" 
                  value={
          ...
          {/* CARD 2: DESCRIPTION & MEDIA (Horizontal Stacked Card) */}
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 pb-2 border-b border-slate-100 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
              ...
              <div className="lg:col-span-8 bg-slate-50/50 rounded-xl p-3.5 border border-slate-100 flex flex-col justify-center">
                ...
                <p className="text-slate-600 text-xs leading-relaxed whitespace-pre-line">
                  {subcategory.description || "No description has been added for this subcategory."}
                </p>
              </div>

              {/* Subcategory Image (Col-span 4) */}
              <div className="lg:col-span-4 flex flex-col justify-center">
                ...
                {subcategory.image ? (
                  ...
                  <div className="h-32 border border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center bg-slate-50 text-slate-400">
                    <Package size={20} className="mb-1 text-slate-300" />
                    <span className="text-[10px] font-semibold">No media uploaded</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* CARD 3: SPECIFICATIONS & REQUIREMENTS (Horizontal Stacked Card) */}
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 pb-2 border-b border-slate-100 flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="bg-teal-50/20 rounded-xl border border-teal-100/50 p-3.5 space-y-2">
                ...
                {subcategory.userRequirements && subcategory.userRequirements.length > 0 ? (
                  ...
                  <p className="text-[10px] italic text-slate-400">No special user requirements listed.</p>
                )}
              </div>
              ...
              <div className="bg-blue-50/15 rounded-xl border border-blue-100/50 p-3.5 space-y-2">
                ...
                {subcategory.equipments && subcategory.equipments.length > 0 ? (
                  ...
                  <p className="text-[10px] italic text-slate-400">No tools mapping specified.</p>
                )}
              </div>
            </div>
          </div>

          {/* CARD 4: INCLUDED SERVICES GRID (Horizontal Stacked Card) */}
          <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm space-y-3">
            <div className="flex items-center justify-between pb-2 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.2 bg-slate-100 text-slate-600 rounded text-[9px] font-bold">
                  {subcategory.includedServices?.length || 0} Items
                </span>
              </div>
            ...
            {subcategory.includedServices && subcategory.includedServices.length > 0 ? (
              ...
              <div className="border border-dashed border-slate-200 rounded-xl p-6 text-center bg-slate-50/50">
                <p className="text-xs text-slate-400 font-semibold">No services have been included in this checklist.</p>
                <Link
                  to={`/home/subcategory/${subcategory._id}/included-services`}
                  className="inline-block mt-2 px-3 py-1 bg-[#0D877F]/10 hover:bg-[#0D877F]/20 text-[#0D877F] rounded text-[10px] font-bold transition-all"
                >
                  Configure checklist
                </Link>
              </div>
